...
;Variables
  ...
  !define PRODUCT_NAME "Electrum WojakCoin"
  !define PRODUCT_EXE_NAME "electrum-wojakcoin"
  !define PRODUCT_WEB_SITE "https://github.com/reallyshadydev/electrum-wojakcoin"
  !define PRODUCT_PUBLISHER "WojakCoin"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\${PRODUCT_EXE_NAME}-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\${PRODUCT_EXE_NAME}-${PRODUCT_VERSION}.exe" "" "$INSTDIR\${PRODUCT_EXE_NAME}-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\${PRODUCT_EXE_NAME}-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\${PRODUCT_EXE_NAME}-${PRODUCT_VERSION}.exe" 0


  ;Links wojakcoin:, bitcoin: and lightning: URIs
  WriteRegStr HKCU "Software\Classes\wojakcoin" "" "URL:wojakcoin Protocol"
  WriteRegStr HKCU "Software\Classes\wojakcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\wojakcoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\wojakcoin\shell\open\command" "" "$\"$INSTDIR\${PRODUCT_EXE_NAME}-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\${PRODUCT_EXE_NAME}-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\${PRODUCT_EXE_NAME}-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\wojakcoin"
